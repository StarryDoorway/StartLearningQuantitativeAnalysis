...
def main():
    ...
    parser.add_argument('--since', type=str, default=None, help='Start time (YYYY-MM-DD or ISO8601). Defaults to trading_config.yaml')
    parser.add_argument('--until', type=str, default=None, help='End time (YYYY-MM-DD or ISO8601). Defaults to now')
    parser.add_argument('--base-dir', type=str, default=None, help='Base data dir, default from trading_config.yaml')
    parser.add_argument('--limit', type=int, default=None, help='Max candles per request')

    args = parser.parse_args()

    settings = load_settings(os.path.join('quant_framework/config', 'trading_config.yaml'))
    base_dir = args.base_dir or settings.get('data', {}).get('base_dir', 'data/raw')
    symbols = args.symbols or settings.get('data', {}).get('default_symbols', [])
    timeframes = args.timeframes or settings.get('data', {}).get('default_timeframes', ['1w'])
    limit = args.limit or int(settings.get('data', {}).get('max_candles_per_request', 100))

    if args.since:
        since_ms = parse_date(args.since)
    else:
        default_start = settings.get('data', {}).get('start_time', '2024-01-01T00:00:00Z')
    ...
    # Debug info
    logger.info(f"Configuration loaded: {bool(settings)}")
    logger.info(f"Using base_dir={base_dir}, symbols={symbols}, timeframes={timeframes}, limit={limit}")
    
    # Check if symbols is empty
    if not symbols:
        logger.error("No symbols specified and no default symbols found in config")
        sys.exit(1)
