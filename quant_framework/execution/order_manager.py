...
class OrderManager:
    ...
    def get_order_statistics(self, strategy_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        filled_orders = [order for order in orders if order.is_filled()]
        avg_fill_price = sum(order.average_fill_price * order.filled_quantity for order in filled_orders) / total_filled_quantity if filled_orders else 0.0
    ...
    def clear_orders(self, strategy_id: Optional[str] = None) -> None:
        """
        Clear orders.
        
        Args:
            strategy_id: Strategy ID (if None, clears all orders)
        """
        if strategy_id:
            order_ids = self.strategy_orders.get(strategy_id, [])
            ...
            del self.strategy_orders[strategy_id]
        else:
            self.orders.clear()
            self.exchange_order_ids.clear()
            self.strategy_orders.clear()
            self.symbol_orders.clear()
        
        self.logger.info(f"Cleared orders for strategy: {strategy_id or 'all'}")
